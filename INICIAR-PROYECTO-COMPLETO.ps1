...
for ($i=45; $i -gt 0; $i--) {
    ...
    # Verificar cada 5 segundos si el backend ya está listo
    if ($i % 5 -eq 0) {
        $backendCheck = netstat -ano | findstr ":8080" | findstr "LISTENING"
        if ($backendCheck) {
            Write-Progress -Activity "Iniciando Backend" -Completed
            Write-Host "`n   ✓ Backend iniciado exitosamente!" -ForegroundColor Green
            break
        }
    }
}
...
if ($backendRunning) {
    ...
    for ($i=30; $i -gt 0; $i--) {
        ...
        if ($i % 5 -eq 0) {
            ...
            if ($frontendCheck) {
                ...
                Write-Host "`n   ✓ Frontend iniciado exitosamente!" -ForegroundColor Green
                break
            }
        }
    }
    ...
    if ($frontendRunning) {
        ...
        Write-Host "⚠ Frontend: Iniciando... (revisa la ventana)" -ForegroundColor Yellow
    }
    ...
    if ($frontendRunning) {
        ...
        Write-Host "⏳ El frontend está compilando..." -ForegroundColor Yellow
    ...
    Write-Host "✗ ERROR: Backend no se inició" -ForegroundColor Red
    ...
    Write-Host "2. Verifica que Java esté instalado: java -version" -ForegroundColor White
